...
def find():
    if request.method == 'POST':
        medicine_query = request.form.get('medicine', '').strip().lower()
        allergies = [a.strip().lower() for a in request.form.getlist('allergies')]
        if 'none' in allergies:
            allergies = []

        # Phase 1: Show list of matching medicines only
        matching_medicines = []
        for row in med_data:
            if len(row) >= 2 and medicine_query in row[1].lower():
                if not any(row[1].lower() in allergy_map.get(a, set()) for a in allergies):
                    matching_medicines.append(row[1])

        matching_medicines = sorted(set(matching_medicines))  # remove duplicates

        return render_template('find.html', step='select', matches=matching_medicines,
                               allergy_list=allergy_list, selected_allergies=allergies)

    return render_template('find.html', step='search', allergy_list=allergy_list)

@app.route('/medicine_details', methods=['POST'])
def medicine_details():
    selected_med = request.form.get('selected_medicine', '').strip().lower()
    allergies = [a.strip().lower() for a in request.form.getlist('allergies')]
    if 'none' in allergies:
        allergies = []

    results, warnings = search_medicine(selected_med, allergies, allergy_map, med_data)
    return render_template('find.html', step='result', results=results,
                           warnings=warnings, allergy_list=allergy_list, selected_allergies=allergies)
